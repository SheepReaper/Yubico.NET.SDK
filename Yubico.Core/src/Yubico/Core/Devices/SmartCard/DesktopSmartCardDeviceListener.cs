...
namespace Yubico.Core.Devices.SmartCard
{
    ...
    {
        private static readonly string[] notificationMS = ["\\\\?\\Pnp\\Notifications"];
        ...
        private bool CheckForUpdates(int timeout, bool usePnpWorkaround)
        {
            ...
            while (ReaderListChangeDetected(ref newStates, usePnpWorkaround))
            {
                ...
                if (addedReaderStates.Length + removedReaderStates.Length > 0)
                {
                    break;
                }
        ...
        {
            SCARD_READER_STATE[] testState = SCARD_READER_STATE.CreateFromReaderNames(notificationMS);
